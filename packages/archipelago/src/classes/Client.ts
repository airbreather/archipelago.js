import type {
	...
	ConnectPacket,
	JSONValue,
} from '@airbreather/archipelago-api';
import {
	clientStatus,
	hintStatus,
} from '@airbreather/archipelago-api/constants';

import { ArgumentError, LoginError, SocketError, UnauthenticatedError } from '../errors';
import { type ClientOptions, defaultClientOptions } from '../interfaces/ClientOptions';
import { type ConnectionOptions, defaultConnectionOptions } from '../interfaces/ConnectionOptions';
import { Item } from './Item';
import { DataPackageManager } from './managers/DataPackageManager';
import { DataStorageManager } from './managers/DataStorageManager';
import { DeathLinkManager } from './managers/DeathLinkManager';
import { ItemsManager } from './managers/ItemsManager';
import { MessageManager } from './managers/MessageManager';
import { type ClientStatus, PlayersManager } from './managers/PlayersManager';
...
export class Client {
	 ...
	 */
	public async login<SlotData extends JSONValue & object>(
		url: URL | string,
		name: string,
		game = '',
	 ...
	 * @remarks The server will automatically set the player's status to {@link clientStatus.disconnected} when all
	 * clients connected to this slot have disconnected, set the status to {@link clientStatus.connected} if a client
	 * connects to this slot when previously set to {@link clientStatus.disconnected}, or ignores any future updates
	 * if ever set to {@link clientStatus.goal}.
	 * @example
	...
	public async scout(locations: number[], createHint: 0 | 1 | 2 = 0): Promise<Item[]> {
		...
		return response.locations.map((item) => new Item(
			...
			item,
			this.players.self,
			this.players.findPlayer(item.player)!),
		);
	}
	 ...
	 */
	public hint(locations: number[], player?: number, status?: typeof hintStatus[keyof typeof hintStatus]): void {
		this.socket.send({ cmd: 'CreateHints', locations, player, status });
	}

	public updateHint(location: { id: number; player: number }, status: typeof hintStatus[keyof typeof hintStatus]) {
		if (!this.authenticated) {
	 ...
	 */
	public bounce(targets: { games?: string[]; slots?: number[]; tags?: string[] }, data: JSONValue & object): void {
		if (!this.authenticated) {
