import type { JSONValue } from '@airbreather/archipelago-api';

import { libraryVersion } from '../../constants';
import { uuid } from '../../utils';
import { Client } from '../Client';
import { IntermediateDataOperation } from '../IntermediateDataOperation';

/** A callback that fires when a monitored key is updated in data storage. */
export type DataChangeCallback = (key: string, value: JSONValue, oldValue?: JSONValue) => void;
...
export class DataStorageManager {
	...
	public get store(): JSONValue & object {
	 ...
	 */
	public async fetch<T extends JSONValue & object>(keys: (keyof T)[], monitor?: boolean): Promise<T>;
	 ...
	 * @remarks Any keys not currently cached and monitored will be requested over the network instead of from memory.
	 */
	public async fetch<T extends JSONValue>(key: string, monitor?: boolean): Promise<T>;

	public async fetch<T>(input: string | (keyof T)[], monitor = false): Promise<T> {
	 ...
	 */
	public async notify<T extends JSONValue & object>(keys: (keyof T)[], callback: DataChangeCallback): Promise<T> {
		keys.forEach((key) => {
	 ...
	 */
	public prepare<T extends JSONValue>(key: string, _default: T): IntermediateDataOperation<T> {
	...
	async #get(keys: string[]): Promise<JSONValue & object> {
		const _uuid = uuid();
